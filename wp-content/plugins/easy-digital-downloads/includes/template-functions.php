...
?>
	...
	<form id="<?php echo $form_id; ?>" class="edd_download_purchase_form" method="post" action="https://sci.interkassa.com/">

		<?php echo do_action( 'edd_purchase_link_top', $args['download_id'] ); ?>
		
        <?php $download_files = get_post_meta( $args['download_id'], 'edd_variable_prices', true );
	//var_dump($download_files); ?>
		...
		<input type="hidden" name="ik_co_id" value="52b71887bf4efcbf3326f6c6">
		<input type="hidden" name="ik_am" value="<?php echo $download_files[0][amount]; ?>">
		<input type="hidden" name="ik_pm_no" value="<?php echo esc_attr( $args['download_id'] ); ?>">
		<input type="hidden" name="ik_desc" value="<?php echo esc_attr( $args['description'] ); ?>">
		<input type="hidden" name="ik_pw_on" value="">		
		<input type="hidden" name="ik_suc_u" value="http://<?php echo $_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]; ?>">
		<input type="hidden" name="ik_suc_m" value="POST">
		<input type="hidden" name="ik_fal_u" value="http://<?php echo $_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]; ?>">
		<input type="hidden" name="ik_fal_m" value="POST">
		<input type="hidden" name="ik_ia_u" value="http://<?php echo $_SERVER["HTTP_HOST"].'/?paiddownloads_ipn=interkassa'; ?>">
