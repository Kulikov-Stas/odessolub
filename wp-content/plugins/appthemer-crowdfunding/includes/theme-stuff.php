...
//var_dump($price);
				...
				<li class="atcf-price-option <?php echo $allgone ? 'inactive' : null; ?>" data-price="<?php echo edd_sanitize_amount( $amount ); ?>-<?php echo $key; ?>">
					<div class="clear">
						...
						?> <span class="pledge-verb"><?php _ex( 'Вложить', 'Pledge verb. (Pledge $5)', 'atcf' ); ?></span> <?php echo edd_currency_filter( edd_format_amount( $amount ) ); ?></label></h3>
						
						<div class="backers">
							<div class="backer-count">
								<i class="icon-user"></i> <?php //printf( _nx( '1 Добродатель', '%1$s Добродателей', $bought, 'number of backers for pledge level', 'atcf' ), $bought ); ?>
								<?php echo $campaign->backers_count().' Добродателей'; ?>
							</div>

							<?php if ( '' != $limit && ! $allgone ) : ?>
								<small class="limit"><?php printf( __( 'Всего нужно %d &mdash; %d осталось', 'atcf' ), $limit, $limit - $campaign->backers_count() ); ?></small>
							<?php elseif ( $allgone ) : ?>
