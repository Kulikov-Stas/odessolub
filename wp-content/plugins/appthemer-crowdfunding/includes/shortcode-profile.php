...
?>
	<h3 class="atcf-profile-section bio"><?php _e( 'Информация профиля', 'atcf' ); ?></h3>
	...
	<form action="" method="post" class="atcf-submit-campaign" enctype="multipart/form-data">
		...
		<p class="atcf-submit-campaign-submit">
			<input type="submit" value="<?php esc_attr_e( 'Обновить', 'atcf' ); ?>">
...
?>
	<p class="atcf-profile-info-first-name">
		<label for="first-name"><?php _e( 'Имя', 'atcf' ); ?></label>
...
?>
	<p class="atcf-profile-info-url">
		<label for="url"><?php _e( 'Сайт', 'atcf' ); ?></label>
...
?>
	<p class="atcf-profile-info-bio">
		<label for="bio"><?php _e( 'Биография', 'atcf' ); ?></label>
...
?>
	...
	<ul class="atcf-profile-contributinos">
		...
		<li>
			...
			<?php printf( _x( '<a href="%s">%s</a> вложено в <a href="%s">%s</a>', 'price for download (payment history)', 'atcf' ), add_query_arg( 'payment_key', $key, get_permalink( $edd_options[ 'success_page' ] ) ), edd_currency_filter( edd_format_amount( $download[ 'price' ] ) ), get_permalink( $download[ 'id' ] ), $download[ 'name' ] ); ?>
			<?php endforeach; ?>
		</li>
		<?php endif; ?>
		<?php endforeach; ?>
