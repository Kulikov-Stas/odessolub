...
<body>
	...
	function front_init() {
		...
		$ip = $_SERVER['REMOTE_ADDR'];			
			if (isset($_POST['ik_co_id']) && isset($_POST['ik_sign']) && ($ip == '85.10.225.99' || $ip == '85.10.225.103')) {
				$request = '';
				...
				$txn_id = $_POST['ik_trn_id'];
				$seller_id = $_POST['ik_co_id'];
				$payer_id = $_POST['ik_x_baggage_fields'];
				$gross_total = $_POST['ik_ps_price'];
				
				$mc_currency = $this->options['interkassa_currency'];
				$first_name = $payer_id;
				$cam = explode('_',$_POST['ik_pm_no']);				

				if ($payment_status == "success") {
					$file_details = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."pd_files WHERE id = '".intval($item_number)."'", ARRAY_A);
					//if (intval($file_details["id"]) == 0) $payment_status = "бред какой-то";
					
						if (strtolower($seller_id) != strtolower($this->options['interkassa_shop_id'])) $payment_status = "не тот id";
						else {
							...
							//if (!$rate) $payment_status = "currency";
							
								$price = number_format($file_details["price"]*$rate, 2, ".", "");
								//if (floatval($gross_total) < $price) $payment_status = "price";
								
								//if (number_format($price, 2, ".", "") != $_POST['ik_am']) $payment_status = "price";	
	/*$dataSet = $_POST['ik_am'].':'.
		$_POST['ik_co_id'].':'.
		$_POST['ik_co_prs_id'].':'.		 
		$_POST['ik_co_rfn'].':'.
		...
		$_POST['ik_pw_via'].':'.
		$_POST['ik_trn_id'].':'.
		$_POST['ik_x_baggage_fields'].':'.
		$this->options['interkassa_secret_key'];*/
//ksort($dataSet, SORT_STRING); // сортируем по ключам в алфавитном порядке элементы массива 
//array_push($dataSet, '9132Dcw4cmz2amgX'); // добавляем в конец массива "секретный ключ" 
//$signString = implode(':', $dataSet); // конкатенируем значения через символ ":" 
//$sign = base64_encode(md5($dataSet, true)); // берем MD5 хэш в бинарном виде по 
	...
	//if ($_POST['ik_sign'] != $sign) $payment_status = "цифровая подпись не та";					
								
							
						}
					
	//$pi = explode('_',$_POST['ik_pm_no']);
	//$payment_id = $pi[0];
				$payment_id = $_POST['ik_pm_no'];
	
	
	$download_files = get_post_meta( $payment_id, 'edd_variable_prices', true );			
    $payment_data = array(
        'price'         => $_POST['ik_am'],
        'date'          => $_POST['ik_inv_prc'],
	//'user_email'    => $purchase_data['user_email'],
        'purchase_key'  => $_POST['ik_sign'],
        'currency'      => $this->options['interkassa_currency'],
        'downloads'     => $_POST['ik_pm_no'],
	//'user_info'     => $purchase_data['user_info'],
        'cart_details'  => $download_files,
        'gateway'       => 'paypal',
        'status'        => 'publish'
     );
...
// в /easy-digital-downloads/includes/payments  functions
    //здесь ошибка возникает из-за того, что $payment_data['cart_details'] должно быть массивом с названием, способом и ценой кампании
    //закачка файла в админке
    $i=1;
    		/*$user_info = array(
			'id'         => '-1',
		...
		edd_insert_payment_note( $payment_id, sprintf( __( 'PayPal Transaction ID: %s', 'edd' ) , $_POST['ik_pm_no'] ) );
		edd_update_payment_status( $payment_id, 'publish' );
		add_action( 'edd_paypal_web_accept', 'edd_process_paypal_web_accept_and_cart' );
		
		
	//$pid = edd_get_payment_amount( 699 );
	 $update_post = array(
		 'ID' => $payment,
	...
	$bought = unserialize($cart_details['bought']);
	$bought++;*/
	//
	update_post_meta( $payment, '_edd_log_payment_id', $payment );
	//update_post_meta( $payment+2, 'strike', 'yes' );			
	...
	function shortcode_handler($_atts) {
		...
		if ($this->check_settings() === true)
		{
			...
			if (intval($sales["sales"]) < $file_details["available_copies"] || $file_details["available_copies"] == 0)
			{
				...
				<script type="text/javascript">
					...
					function paiddownloads_'.$button_id.'() {
						...
						if (active_'.$button_id.' == "interkassa_'.$button_id.'") {
							...
							jQuery("#ik_x_baggage_fields_'.$button_id.'").val(paiddownloads_email);
						}
				...
				<form id="payment" name="payment" action="https://sci.interkassa.com/" method="post" style="display:none;" enctype="utf-8">
					<input type="hidden" name="ik_co_id" value="'.$this->options['interkassa_shop_id'].'">
					<input type="hidden" name="ik_am" value="'.number_format($price, 2, ".", "").'">
					<input type="hidden" name="ik_pm_no" value="'.$file_details["id"].'_'.time().'">
					<input type="hidden" name="ik_desc" value="'.htmlspecialchars($file_details["title"], ENT_QUOTES).'">
					<input type="hidden" name="ik_pw_on" value="">
					<input type="hidden" id="ik_x_baggage_fields_'.$button_id.'" name="ik_x_baggage_fields" value="">
					<input type="hidden" name="ik_suc_u" value="'.$return_url.'">
					<input type="hidden" name="ik_suc_m" value="POST">
					<input type="hidden" name="ik_fal_u" value="http://'.$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"].'">
					<input type="hidden" name="ik_fal_m" value="POST">
					<input type="hidden" name="ik_ia_u" value="'.get_bloginfo("wpurl").'/?paiddownloads_ipn=interkassa">
